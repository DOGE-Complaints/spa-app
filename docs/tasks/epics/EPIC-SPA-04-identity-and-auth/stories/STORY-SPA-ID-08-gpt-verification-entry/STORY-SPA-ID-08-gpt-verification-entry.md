...
- **Epic:** EPIC-SPA-04 Identity & Auth · **Волна 4 (GPT bridge)**
- **Status:** Done
- **Wave:** `pkg-000025` (closed 2026-07-02)
- **source:** [`spa-app/docs/tasks/backlog-stories/identity-auth/STORY-SPA-ID-08-gpt-verification-entry.md`](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-08-gpt-verification-entry.md)
- **Decision Ref:** [`../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-08-gpt-verification-entry.md`](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-08-gpt-verification-entry.md); [mockup-120-gpt-story-authorization-flow-state-sheet-spec.md](../../../../../../UX/mockups/epic-04/mockup-120-gpt-story-authorization-flow-state-sheet-spec.md); [identity-auth-api-validation-2026-06-27.md](../../../../../../analysis/identity-auth-api-validation-2026-06-27.md); [identity-gateway-integration-notes-2026-06-23.md](../../../../../../analysis/identity-gateway-integration-notes-2026-06-23.md)
- **Источник:** [epic-04 S04-2](../../../../../../backlog-stories/inbound/epic-04.md), [identity-frontend FR-FE-006 / S6 / S7](../../../../../../../docs/Identity/identity-frontend.md)
...
- Спек: [mockup-120-gpt-story-authorization-flow-state-sheet-spec.md](../../../../../../UX/mockups/epic-04/mockup-120-gpt-story-authorization-flow-state-sheet-spec.md)
- PNG: [mockup-120-gpt-story-authorization-flow-state-sheet-spec.png](../../../../../../UX/mockups/epic-04/mockup-120-gpt-story-authorization-flow-state-sheet-spec.png) — единственный SSOT GPT-моста (бывший M61 удалён как перекрытый, см. [README §4](../../../../../../backlog-stories/identity-auth/README.md)).
...
- **FR-08.3** Нет сессии → login/signup (переиспользовать [ID-01](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-01-web-authentication.md)); signup email-first (телефон позже).
- **FR-08.4** Аутентифицирован, `phone_verified=false` → disclosure + verify (переиспользовать [ID-04](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-04-phone-verification-flow.md)).
- **FR-08.5** Success → «Verified civic participant» + **«Return to ChatGPT»** (+ Open Profile); already-verified → сразу «You're ready».
- **FR-08.6** Error side-panel — те же коды, что [ID-05](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-05-verification-error-states.md) (компактно).
...
2. SPA `/login`: вход/регистрация через **Supabase Auth** (reuse [ID-01](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-01-web-authentication.md)). Получив Supabase JWT, SPA → `POST /oauth/authorize/complete` (Bearer + `{"oauth_request_id":"<id>"}`).
3. Ответ identity:
...
4. На `/verify?context=` — phone-флоу (reuse [ID-04](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-04-phone-verification-flow.md): `/auth/phone/request` → `/confirm`). После `phone_verified=true` SPA **повторяет** `POST /oauth/authorize/complete` с тем же `oauth_request_id` → теперь happy-path 302 в ChatGPT.
...
## Зависимости
- Переиспользует [ID-01](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-01-web-authentication.md), [ID-04](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-04-phone-verification-flow.md), [ID-05](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-05-verification-error-states.md), [ID-03](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-03-civic-status-component.md). Draft-резюм паттерн зеркалит [ID-06](../../../../../../backlog-stories/identity-auth/STORY-SPA-ID-06-protected-action-gate.md). Backend OAuth (OAUTH-01…04) — **построен**, не блокер.
...
- [x] Точка входа `/login?oauth_request_id=<id>` (из `/oauth/authorize`-redirect) проводит login/signup и вызывает `POST /oauth/authorize/complete`.
- [x] Happy-path: `/complete` → 302 в ChatGPT (`redirect_uri?code&state`); экран «Return to ChatGPT».
- [x] `verification_required` (403, плоский `{error,reason,verify_url}`) → SPA уходит на `verify_url` (`/verify?context=`), проводит phone-verify, **повторяет** `/complete` → 302 в ChatGPT.
- [x] Already-verified → сразу 302 code, без OTP («You're ready»).
- [x] Story-draft preserved виден на всех состояниях; «работа не потеряна».
- [x] Credentials/номер/OTP не проходят через GPT (только наш веб).
- [x] Все строки локализованы (et/ru/en) через `t()`; нет хардкод-английского (FR-08.9).
