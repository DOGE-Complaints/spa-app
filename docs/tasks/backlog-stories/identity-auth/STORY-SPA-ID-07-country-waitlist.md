...
- **FR-07.6** Триггер: `COUNTRY_NOT_ALLOWED` от `POST /auth/phone/request` (срабатывает по **введённому dial-префиксу**, не по IP) — приходит из [ID-05](STORY-SPA-ID-05-verification-error-states.md). **Решение 2026-06-23:** страна определяется из набранного номера; IP-geo нет.
...
## Зависимости
- Входит из [ID-05](STORY-SPA-ID-05-verification-error-states.md) (Country Not Allowed → этот flow). Не зависит от phone-flow happy-path.
