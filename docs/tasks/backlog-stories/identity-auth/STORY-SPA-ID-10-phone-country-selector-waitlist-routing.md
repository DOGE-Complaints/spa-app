...
- **Epic:** [EPIC-SPA-04 Identity & Auth](README.md) · **Волна 3 (Protected action)**
- **Status:** Done (pkg-000023, 2026-06-30)
...
- Контекст: [M32 §State B](../../../UX/mockups/epic-04/mockup-32-phone-verification-flow-sheet-spec.md) (исходное требование селектора), переход в [M123 Country Waitlist](../../../UX/mockups/epic-04/mockup-123-country-waitlist-state-sheet-spec.md). Формат/валидация номера — отдельно [M127/ID-11](STORY-SPA-ID-11-per-country-phone-format-validation.md).
...
## Зависимости
- Меняет `PhoneInputPanel` (ID-04) и `VerifyPage` waitlist-routing (ID-07). Предшествует [ID-11](STORY-SPA-ID-11-per-country-phone-format-validation.md) (формат-валидация строится на этом селекторе).

## Вне scope
- **Полная валидация формата номера по стране — [ID-11](STORY-SPA-ID-11-per-country-phone-format-validation.md).** Реальный IP-geo. Расширение supported-стран на бэкенде (config/ops).
...
- [ ] Supported = `+372` (FE-зеркало бэкенда) через `isSupportedDialPrefix`.
- [ ] Эстония → OTP happy-path без изменений.
- [ ] Не-Эстония → уведомление + CTA «Join Waitlist», без вызова `/auth/phone/request`.
- [ ] «Join Waitlist» ведёт в ID-07 со страной из **выбора** (ID-07 F2 закрыт); телефон не хранится.
- [ ] Fallback `COUNTRY_NOT_ALLOWED`→waitlist сохранён.
- [ ] Все строки локализованы (et/ru/en) через `t()`; нет хардкод-английского (FR-10.7).
- [ ] `npx vitest run` green; тесты supported/unsupported routing + parity ключей.
