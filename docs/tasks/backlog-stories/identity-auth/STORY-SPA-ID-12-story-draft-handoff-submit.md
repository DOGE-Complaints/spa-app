...
- **Epic:** [EPIC-SPA-04 Identity & Auth](README.md)
- **Status:** ✅ Done
- **Severity:** 🔴 HIGH (клиентская половина M-3; чинит M-7)
- **Источник:** [`mvp-integration-plan-2026-07-02.md` §2,§4](../../../../../doge-identity-service/docs/analysis/mvp-integration-plan-2026-07-02.md)
...
- **Иконки/визуалы (SSOT имён файлов):** [STORY-SPA-ID-12-icon-assets.md](STORY-SPA-ID-12-icon-assets.md) — каталог всех функциональных иконок M128 (контекст ↔ имя файла ↔ промпт на прозрачном растре). **Имена файлов иконок брать строго отсюда** (см. Scope §I); путь в коде — `/icons/story-handoff/ic-<name>.png`.
...
- **I. Иконки/ассеты (обязательно):** все иконки экранов брать из каталога [STORY-SPA-ID-12-icon-assets.md](STORY-SPA-ID-12-icon-assets.md) — это **SSOT имён файлов**, **не выдумывать** и не заводить свои имена.
...
| **T03** | `StorySubmitPage` (rename + rewrite compose): машина состояний A/B/C/E0/F/G/H; `useSearchParams`; удалить локальный редактор; **иконки состояний — по именам из [icon-assets.md](STORY-SPA-ID-12-icon-assets.md)** (`/icons/story-handoff/ic-*.png`, Scope §I) | `StorySubmitPage.jsx` (ex-Compose) |
...
| **T06** | Route rename `/story/submit`: `PROTECTED_PREFIXES`, ссылки, deploy; «создать историю»→`VITE_STORY_GPT_URL`; временный redirect `/story/compose`→`/story/submit` на период cutover (removable) | router, shell/board |
| **T07** | L10N `storyHandoff.*` (et/ru/en) + FLAT_KEYS; reuse `gptBridge.*` | `identityDictionary.js` |
| **T08** | Vitest: preview(200)/expired(404)/no-session(401→redirect)/submit(202)/verify-interpose(403→auto-resubmit)/service-down(503)/empty(no draft_id) | `__tests__` |
| **T09** | UI-anchor на артборд **M128** (готов, validated); лейблы `storyHandoff.*` синхронизированы с M128; иконки — по каталогу [icon-assets.md](STORY-SPA-ID-12-icon-assets.md) в `public/icons/story-handoff/` | mockup ref |
...
## Acceptance Criteria
- [x] `?draft_id` → `GET /story-drafts/{id}` под Bearer → полный original-language предпросмотр (реальный вызов, не mock); 401→login-redirect (draft сохранён); 404→«истёк»; нет draft_id→empty-state «создать в GPT».
- [x] Предпросмотр доступен **до** phone-verify (read active-only); контент только на preview, не на verify.
- [x] Подтверждение → `POST /story-drafts/{id}/submit`: 202→успех (submission_id, «на модерации»); 403→verify→**авто-ресабмит**; 401→login-redirect; 503→ретрай; 404→«истёк».
- [x] Браузер **не** вызывает `POST /story-drafts`.
- [x] M-7 закрыт: нет вызовов с неверным auth/несуществующими путями.
- [x] Локальный редактор удалён; «создать историю» ведёт на `VITE_STORY_GPT_URL`.
- [x] Роут `/story/submit` (rename применён во всех швах); временный redirect старого пути на cutover.
- [x] Все строки chrome локализованы (et/ru/en); контент истории — на языке оригинала; нет хардкод-английского.
...
- **Иконки:** файлы → `public/icons/story-handoff/` (имена из [icon-assets.md](STORY-SPA-ID-12-icon-assets.md)); в коде — URL `/icons/story-handoff/ic-*.png` (Scope §I).
