/**
 * CAB-04 Story Activity screenshots → T01 ui-baseline/
 * Usage:
 ...
 * Env: CABINET_STORY_URL=http://127.0.0.1:4173 to reuse existing Vite.
 */
...
const UI_BASELINE = path.resolve(
  SPA_ROOT,
  'docs/tasks/epics/EPIC-SPA-07-user-cabinet/stories/STORY-SPA-CAB-04-story-activity-card/task-spa-cab-04-t01-mount-story-activity-card/ui-baseline',
)

const BASE = process.env.CABINET_STORY_URL ?? 'http://127.0.0.1:4173'
const USE_EXISTING = Boolean(process.env.CABINET_STORY_URL)
const PHASE = process.env.CAB04_PHASE === 'pre-implement' ? 'pre-implement' : 'post-implement'
const VIEWPORT = { width: 1536, height: 1024 }

const POST_STATES = [
  { letter: 'a', slug: 'active-history', preview: 'active' },
  { letter: 'b', slug: 'empty-go-to-board', preview: 'empty' },
  { letter: 'c', slug: 'draft-coming-soon', preview: 'draft' },
  { letter: 'd', slug: 'verify-required', preview: 'verify' },
  { letter: 'e', slug: 'unavailable', preview: 'unavailable' },
]
  ...
  await page.evaluate(() => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        user: { id: 'mock-user-cab04', email: 'cab04@example.com' },
      }),
    )
...
async function openProfileWithPreview(page, preview) {
  ...
  await page.waitForSelector('[data-testid="cabinet-slot-story"]', { timeout: 30000 })
  await sleep(400)
}
...
async function run() {
  ...
  try {
    if (PHASE === 'pre-implement') {
      ...
      await shot(page, 'b-story-slot-placeholder-1536x1024.png')
    } else {
      for (const state of POST_STATES) {
        await openProfileWithPreview(page, state.preview)
        await page.waitForSelector('[data-story-activity-card]', { timeout: 15000 })
